...
<?php
 ...
 $message = '<tr><td>'.$onlydate.'</td><td>'.$dteEnd->format('h:i A').'</td><td>'.$interval.'</td><td>'.$count.'</td><td>Rs. '.$grandtotal.'</td></tr>';
		...
		<h3 class="subtitle-text">Today's Team Effort</span></h3>
<table>
  <tr class="header">
    <th>Date</th>
	<th>No. of Orders</th>
...
<?php
        ...
        $grandtotal = 0;
		$count = 0;
		$results2 = $db->query("SELECT Grand_Total, DATE_FORMAT(Order_Date, '%Y-%m-%d') from orders WHERE DATE(Order_Date) = '$today' ");
			while($row2 = $results2->fetch_assoc())
		{
			$count   = $count+1;
			$grandtotal = intval($row2["Grand_Total"])+$grandtotal;
		}
            $message = '<tr><td>'.$onlydate.'</td><td>'.$count.'</td><td>Rs. '.$grandtotal.'</td></tr>';
		    echo $message;
		?>
		</table><br>
		<a href="logout.php" title="Log Out" class="red-button-2">Log Out</a>
</div>
